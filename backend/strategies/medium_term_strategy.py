...
class MediumTermStrategy(BaseStrategy):
    ...
    def __init__(self):
        logger.info("Initializing MediumTermStrategy")
        super().__init__()
    ...
    def get_position_size(self):
        if self.params.position_size_type == 'fixed':
            # 确保固定数量至少为100股
            return max(100, self.params.position_size)
        else:  # percentage
            available_cash = self.broker.getcash()
            current_price = self.data.close[0]
            # 计算按比例可以买入的股数
            shares = int((available_cash * self.params.position_size / 100) / current_price)
            # 向下取整到最接近的100的倍数，并确保至少为100股
    ...
    def next(self):
        ...
        # 入场信号
        if self.ma5_cross_ma10 > 0 and \
                self.rsi < self.params.rsi_overbought:
            self.signal = True

        # 如果有信号且MA10上穿MA20，则做多
        if self.signal and self.ma10_cross_ma20 > 0 and not self.position:
