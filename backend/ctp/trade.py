...
#     data0 = store.getdata(dataname='ag2501.SHFE', timeframe=bt.TimeFrame.Minutes, #注意符号必须带交易所代码。
#                           num_init_backfill=100 if is_trading_period() else 0)     #初始回填bar数，使用TEST服务器进行模拟实盘时，要设为0

#     data1 = store.getdata(dataname='rb2501.SHFE', timeframe=bt.TimeFrame.Minutes, #注意符号必须带交易所代码。
#                           num_init_backfill=100 if is_trading_period() else 0)     #初始回填bar数，使用TEST服务器进行模拟实盘时，要设为0
